...
data.med.ind$youth <- data.ind$youth_visits
data.med.ind$adults <- data.ind$parents_visits + data.ind$add_parents_visits
data.med.ind$kids <- data.ind$kids_visits + data.ind$add_kids_visits +
  data.ind$dep_visits
data.med.ind$department <- data.ind$dep_visits
data.med.ind$disabled <- data.ind$disabled
data.med.ind$visitors <- data.ind$visits_total
...
attr(data.med.ind[, 16], "label") <- "Прочие"
...
# Экспорт массива
# save(data.med.ind, file = "~/aism/2018/data_medical_ind2018.rda")
