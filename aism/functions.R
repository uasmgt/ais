...
# информация о кол-ве воспитателей ----
GetStaff <- function(x){
...
# недозаезды (индивидуальный отдых) ----
CountInd <- function(x){
...
# чтение листов с отрядами (для индивидуального отдыха)
...
# медообращения (с 2019 года) ----
GetMedicalCur <- function(x){
  x <- read.xlsx(x, sheet = 1)
  trm01 <- x[4, ncol(x)]
  trm02 <- x[5, ncol(x)]
  trm03 <- x[6, ncol(x)]
...
# медобращения (до 2019 года) ----
GetMedicalOld <- function(x){
  x <- read.xlsx(x, sheet = 1)
  trm01 <- x[4, ncol(x)]
  trm02 <- x[5, ncol(x)]
  trm03 <- x[6, ncol(x)]
