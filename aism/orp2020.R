...
# Создание массивов ----------------------------------------------------
setwd("~/aism/2020/arrivals_orp")

file.names <- list.files(path = "./", recursive = TRUE, 
                         pattern = "*.xlsx") # чтение названий xlsx-файлов
data.arrivals <- lapply(file.names, Countorp)
data.arrivals <- data.frame(matrix(unlist(data.arrivals), 
                                   nrow = length(data.arrivals), 
                                   byrow = TRUE))
colnames(data.arrivals) <- c("orpily_visits", "kids_visits", "disabled", 
                             "orphans", "needy", "parents_visits", "orpily_non",
...
source("~/git/ais/aism/labels_orp.R", encoding = "UTF-8")

# Сохранение данных ----------------------------------------------------
# orp2020 <- dataset[c(1:3, 5:38)]
# med.orp2020 <- dataset[c(1:3, 5:6, 32, 39:62)]
data.orp2020 <- dataset

# для анализа в R
save(data.orp2020, file = "~/data/data_orp2020.rda")
# для анализа в Excel
write.csv2(data.orp2020, file = "~/data/data_orp2020.csv")
